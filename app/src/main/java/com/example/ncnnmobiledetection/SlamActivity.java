...
public class SlamActivity extends UnityPlayerActivity {
//    public static final String TAG = "XVSDK Demo";
    ...
    public StreamHandler mMainHandler;
    public boolean runClassifier = false;

    public Handler handler = new Handler();

    public NcnnMobileDetection ncnnDet = new NcnnMobileDetection();
    public static SlamActivity instance = getInstance();  //单例类

    public static SlamActivity getInstance() {
        if (instance == null) {
            instance = new SlamActivity();
        }
        return instance;
    }
...
//        }
    ...
    public void init() {
        Log.d("SlamActivity", "init");
        UnityPlayer.currentActivity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Log.d("SlamActivity", "start");
                boolean ret_init = ncnnDet.Init(UnityPlayer.currentActivity.getAssets());
                if (!ret_init) {
                    Log.d("SlamActivity", "yolov5ncnn Init failed");
                }
